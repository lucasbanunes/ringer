...
def hist1d_from_bins_df(binned_data, cols, ax, xlabel=None, ylabel=None, legend=True, xlim=None, ylim=None):
    bin_width = binned_data['bin_end'] - binned_data['bin_start']
    for col in cols:
        ax.bar(binned_data['bin_start'], binned_data[col], align='edge', width=bin_width, label=col, alpha=0.4, bottom=0, aa=True)
    if ylim is None:
        ax.set_ylim(0, binned_data[cols].max().max()*1.05)
    else:
        ax.set_ylim(ylim)
    if xlim:
        ax.set_xlim(xlim)
...
def subsample_bins_df(binned_data, subsample):
    subsampled_binned = binned_data.reset_index()
    subsampled_binned['group'] = subsampled_binned['index'] // subsample
    data_cols = [col for col in binned_data.columns if col != 'bin_start' and col != 'bin_end']
    agg_dict = {col: 'sum' for col in data_cols}
    agg_dict['bin_start'] = 'min'
    agg_dict['bin_end'] = 'max'
    subsampled_binned = subsampled_binned.groupby('group') \
        .agg(agg_dict).dropna().reset_index(drop=True)
    # Renormalizing
    for col in data_cols:
        area = (subsampled_binned[col] * (subsampled_binned['bin_end'] - subsampled_binned['bin_start'])).sum()
        subsampled_binned[col] = subsampled_binned[col]/area
    return subsampled_binned