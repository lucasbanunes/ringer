...
def get_wasserstein_distances(
    ...
    ss_filters: Mapping[str, Callable[[pd.Series], pd.Series]],
    description: str,
    ss_to_analyze: pd.Series
        ) -> pd.DataFrame:
    ...
    wass_distances = pd.DataFrame(
        index=ss_to_analyze.index,
        columns=cols,
        dtype=float
    )
    wass_distances.index.name = 'name'

    for ss_name, ss_col in ss_to_analyze.items():
...
boosted_data = NamedDatasetLoader('MC16 Boosted', test) \
...
collision_data = NamedDatasetLoader('2017 Medium', test) \
...
for ifold in range(n_folds):
    collision_fold_idxs: tuple = next(collision_split)
    boosted_fold_idxs: tuple = next(boosted_split)
    for i, fold_type in enumerate(FOLD_TYPES):
        fold_data = {
            'boosted': boosted_data.loc[boosted_fold_idxs[i]],
            'el': collision_data.loc[collision_fold_idxs[i] & el_label],
            'jet': collision_data.loc[collision_fold_idxs[i] & jet_label]
        }
        wass_distances = get_wasserstein_distances(
            fold_data,
            ss_filters,
            description=f'fold_{ifold}_{fold_type}',
